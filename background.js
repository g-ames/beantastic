...
const performanceLimit = 1;
const oldSetInterval = window.setInterval;

setInterval = function(a, b) {
    if(b == 0) {
        console.log("si call", b, performanceLimit - 1)
        return oldSetInterval(a, performanceLimit - 1)
    } else {
        console.log("si call", b, performanceLimit)
        return oldSetInterval(a, b * performanceLimit)
    }
}

const beansBackground = document.getElementById("background-beans");
const beansCan = document.getElementById("beans-can");
const infoBoard = document.getElementById("info-board");
const shopButton = document.getElementById("shop-button");
const shopDialog = document.getElementById("shop-dialog");
const purchasablesDiv = document.getElementById("purchasables-div");
const settingsDialog = document.getElementById("settings-dialog");
const beanMinigame = document.getElementById("bean-minigame");
const miniGameGemsContinueDialog = document.getElementById("mini-game-gems-continue-dialog");
const specialBeansCompleteList = [
    "Mystic Bean",
    "Shadow Bean",
    "Glowing Bean",
    "Inferno Bean",
...
const beansCanAnimTickTime = 100;
const beansMargin = 6;
const maxBeans = 1000;
...
let invisibleStats = JSON.parse(localStorage.getItem("invisibleStats")) || {
    "Special Beans List" : []
};
...
let stats = JSON.parse(localStorage.getItem("stats")) || {
    ...
    "Gems": 0,
    "Auto Beans": 0,
    "Special Beans" : invisibleStats["Special Beans List"].length,
};

const purchasables = [
    {
        "name": "Auto Bean",
        "action": createBPSBean,
        "price": 0.50,
        "priceMultiplier": 3
    },
    {
...
const beanMinigames = [whoSpilledTheBeansMiniGame];

function getRandomPosition() {
    return `${Math.round(Math.random() * 100 - beansMargin) + beansMargin / 2}%`;
}

function createBPSBean() {
    if (beans.length < maxBeans) {
        let newBean = document.createElement("img");
        newBean.draggable = false;
        newBean.src = "kdbean.png";
        newBean.style.cssText = `
            position: absolute;
            width: 3%;
            height: auto;
            top: ${getRandomPosition()};
            left: ${getRandomPosition()};
            transition: ${1*performanceLimit}s ${['linear', 'ease', 'ease-in', 'ease-out', 'ease-in-out'][Math.floor(Math.random() * 5)]};
        `;
        beansBackground.appendChild(newBean);
        beans.push(newBean);
    }
    stats["Auto Beans"]++;
}

function createGoldenBean() {
    let goldenBean = document.createElement("img");
    goldenBean.draggable = false;
    goldenBean.src = "kdbean.png";
    goldenBean.style.cssText = `
        filter: sepia(1) blur(2px) contrast(1.5);
    ...
    setTimeout(() => {
        goldenBean.style.top = getRandomPosition();
        goldenBean.style.left = getRandomPosition();
        goldenBean.style.opacity = 0.5;
    }, 10);

    setTimeout(() => {
        goldenBean.style.top = getRandomPosition();
        goldenBean.style.left = getRandomPosition();
        goldenBean.style.opacity = 0;
        setTimeout(() => goldenBean.remove(), 2000);
    }, 1000);
}

function beansTick() {
    beans.forEach(bean => {
        bean.style.top = getRandomPosition();
        bean.style.left = getRandomPosition();
    });
    stats.Beans += stats["Auto Beans"];
    updateInfoBoard();
}

function beanCansTick() {
    const time = Date.now() / 1000;
    beansCan.style.rotate = `${Math.sin(time) * 1}deg`;
    beansCan.style.transform = `scale(${(Math.sin(time) * 0.1) + 1})`;
}

function updateInfoBoard() {
    stats["Special Beans"] = invisibleStats["Special Beans List"].length
    stats.Money = Math.round(stats.Money * 100) / 100;
    const betterStats = Object.fromEntries(Object.entries(stats).filter(([key, value]) => value !== 0 && value !== null && value !== undefined));
    infoBoard.textContent = jsyaml.dump(betterStats).replaceAll("{}", "...");
}

function updatePurchasables() {
    purchasablesDiv.innerHTML = "<hr>";
    purchasables.forEach(purchasable => {
        const header = document.createElement("h2");
        header.textContent = purchasable.name;
        purchasablesDiv.appendChild(header);

        const buyButton = document.createElement("button");
        buyButton.textContent = `Buy ${purchasable.name} for $${purchasable.price}`;
        buyButton.onclick = () => {
            if (purchasable.price <= stats.Money) {
                stats.Money -= purchasable.price;
                cachedFunctions.push(purchasable.action);
            } else {
                alert("You don't have enough Money!");
            }
        };
        purchasablesDiv.appendChild(buyButton);

        const maxBuyButton = document.createElement("button");
        maxBuyButton.textContent = `Max buy ${purchasable.name}`;
        maxBuyButton.onclick = () => {
            while (purchasable.price <= stats.Money) {
                stats.Money -= purchasable.price;
                cachedFunctions.push(purchasable.action);
            }
        };
        purchasablesDiv.appendChild(maxBuyButton);
        purchasablesDiv.appendChild(document.createElement("hr"));
    });
}

function whoSpilledTheBeansMiniGame() {
    beanMinigame.style.cssText = `
        width: 100%;
        height: 100%;
        text-align: center;
    `;
    ...
    giveUpButton.onclick = () => {
        beanMinigame.close();
        beanMinigame.innerHTML = '';
    };

    beanMinigame.appendChild(minigameCanvas);
    beanMinigame.appendChild(giveUpButton);
    beanMinigame.showModal();
}

setInterval(beansTick, 1000);
beansCan.style.transition = `${(beansCanAnimTickTime*(beansCanAnimTickTime*0.05))*performanceLimit}ms`
setInterval(beanCansTick, beansCanAnimTickTime);

setInterval(() => {
    localStorage.setItem("stats", JSON.stringify(stats));
    localStorage.setItem("invisibleStats", JSON.stringify(invisibleStats));
    console.log("saved.");
}, 3000);

if (stats.Beans > 20) {
    shopButton.style.visibility = "";
} else {
    const checkIfShopReady = setInterval(() => {
        if (stats.Beans > 20) {
            shopButton.style.visibility = "";
            clearInterval(checkIfShopReady);
        }
    }, 100);
}
...
beansCan.onclick = () => {
    stats.Beans++;
    updateInfoBoard();
    beansCan.style.transform = `scale(${(Math.sin(Date.now() / 1000) * 0.1) + 2})`;
};

for (let i = 0; i < 10; i++) {
    setInterval(() => {
        while (cachedFunctions.length) {
            const func = cachedFunctions.pop();
            if (typeof func === 'function') {
                func();
            } else {
                console.warn(`${func}, CachedFunction, nonfunction!`);
            }
        }
    }, 0);
}
...
setInterval(() => {
    miniGameGemsContinueDialog.style.display = stats.Gems >= 3 ? '' : 'none';
}, 1000);
...
// unsmooth 0.032970693215245615
// smooth 0.03298252011907909
// smooth is -0.000011826903833472846 worse

// 10x more its 0.0035335913539377294
// smooth 10x more 0.002789554792726352
// smooth is 0.0007440365612113773 better

// 0.0014637337157310212
// 0.0007259434289834579
// smoother by 0.0007377902867475633 nearly half (off by 0.00001184685776410544)

// 0.000625478245950351
// 0.0009256428731340732
// "smoothed" -0.0003001646271837223 worse